...
class Encoder(nn.Module):
    ...
    def __init__(self, dim=24):
        ...
        self.shape = (1, 28, 28)
        self.dim = dim
        self.dropout = 0.01625 #0.03125 #0.125 #
        ...
        self.getmu = nn.Sequential(
                #nn.Dropout(p=self.dropout),
                nn.utils.weight_norm(nn.Linear(4 * 3 * 3 * self.dim, 3*3*self.dim), dim=None),
                nn.PReLU(),
                nn.utils.weight_norm(nn.Linear(3 * 3 * self.dim, self.dim), dim=None),
        )
...
class LoglossPair(nn.Module):
    #can't turn dropout off completely because otherwise the loss -> NaN....
    #batchnorm does not seem to help things...
    def __init__(self, dim=4):
        super(LoglossPair, self).__init__()
        self.dim = dim

        self.output = nn.Sequential(
                nn.Linear(1, self.dim),
                #nn.utils.weight_norm(nn.Linear(1, self.dim), dim=None),
                nn.ReLU(True),
                #nn.utils.weight_norm(nn.Linear(self.dim, self.dim), dim=None),
                #nn.ReLU(True),
                #nn.utils.weight_norm(nn.Linear(self.dim, self.dim), dim=None),
                #nn.ReLU(True),
                #nn.utils.weight_norm(nn.Linear(self.dim, 1), dim=None),
    ...
    def forward(self, diff):
        #diff = torch.mean(torch.pow(input0 - input1, 2), 1)
        diff = diff.view(-1, 1)
        out = self.output(diff)
        return out.view(-1)
